...
} & CustomEventInstanceProperties<Def>;`} /> for that platform. The instance gives you `user()` and `space()` resolvers, plus access to any custom events the provider emits.
