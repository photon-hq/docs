---
title: "Custom Events and Lifecycle"
...
Fusor-backed providers can emit non-message events (presence, read receipts, delivery status) into typed event streams using `fusorEvent`. The helper returns a <TypeTooltip name="FusorEvent" type={`interface FusorEvent<TName extends string = string, TData = unknown> {
    readonly data: TData;
    readonly name: TName;
