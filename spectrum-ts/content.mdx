...
Spectrum exposes a family of content builders — `text`, `attachment`, `voice`, `contact`, `richlink`, `poll`, `group`, and `custom` — plus a string shortcut that's equivalent to `text()`. Any API that takes a <TypeTooltip name="ContentInput" type={`type ContentInput = string | ContentBuilder;`} /> accepts a plain string or a <TypeTooltip name="ContentBuilder" type={`interface ContentBuilder {
