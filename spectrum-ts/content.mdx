...
Spectrum exposes a family of content builders — `text`, `markdown`, `attachment`, `voice`, `contact`, `richlink`, `poll`, `group`, `custom`, `reaction`, `reply`, `edit`, `unsend`, `typing`, `rename`, and `avatar` — plus a string shortcut that's equivalent to `text()`. Any API that takes a <TypeTooltip name="ContentInput" type={`type ContentInput = string | ContentBuilder;`} /> accepts a plain string or a <TypeTooltip name="ContentBuilder" type={`interface ContentBuilder {
...
Every resolved attachment carries a stable `id`. When the attachment originates from a provider, the ID preserves the provider-native identifier (iMessage GUID, Slack file ID, WhatsApp media ID). For outbound attachments built with `attachment()`, a random UUID is assigned unless you pass `options.id`:
...
Send a poll with a title and a list of choices. Each choice can be a plain string or a <TypeTooltip name="PollChoiceInput" type={`type PollChoiceInput = string | {
    title: string;
