...
| API endpoint pages | a remote OpenAPI schema referenced from `docs.base.json` | Mintlify, at render time             |
...
fetched live at render time. Nothing is written to the working tree, so there is
no sync step and nothing to gitignore.
