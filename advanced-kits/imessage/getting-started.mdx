...
description: "Install the iMessage Kit and send your first message in minutes"
---
...
|---|---|---|
| `address` | `string` | The server address to connect to (e.g., `"localhost:50051"`). |
| `autoIdempotency` | `boolean` | When `true`, automatically generates a unique key for each request to prevent duplicate operations. |
| `retry` | `boolean \| RetryOptions` | When `true`, enables automatic retries. Accepts a `RetryOptions` object for fine-grained control. |
| `timeout` | `number` | Request timeout in milliseconds. |
| `tls` | `boolean` | When `true`, uses a TLS-encrypted connection to the server. |
| `token` | `string \| (() =&gt; Promise&lt;string&gt;)` | Authentication token. Pass a function that returns `Promise<string>` to provide the token dynamically. |
