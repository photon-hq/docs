...
| `sticker` | `{ target: MessageGuid; placement?: StickerPlacement }` | Send as a sticker on another message |
| `formatting` | `TextFormatInput[]` | Bold, italic, underline, or text effects |
| `service` | `string` | Override service (`"iMessage"` or `"SMS"`) |
...
// Tapback
await im.messages.react(chat, messageGuid, Reaction.heart);
await im.messages.react(chat, messageGuid, Reaction.thumbsUp, { partIndex: 1 });

// Emoji reaction (iOS 17+)
await im.messages.reactEmoji(chat, messageGuid, "🔥");

// Remove a tapback
await im.messages.unreact(chat, messageGuid, Reaction.heart);
```
...
| `sort` | `SortDirection` | `"asc"` or `"desc"` |
