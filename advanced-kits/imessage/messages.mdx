...
| `afterCursor` | `string` | Resume from a stream event cursor. Overrides `after` and forces ascending sort. |
...
| Type | Extra fields |
|---|---|
| `message.sent` | `message`, `chatGuid`, `clientMessageId?`, `cursor?` |
| `message.received` | `message`, `chatGuid`, `cursor?` |
| `message.updated` | `message`, `chatGuid`, `updateType` (`edited` \| `unsent` \| `notified` \| `reaction`), `cursor?` |
| `message.sendError` | `chatGuid`, `errorCode`, `errorMessage`, `clientMessageId?`, `cursor?` |
