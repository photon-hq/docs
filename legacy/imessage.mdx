...
```typescript
sdk.on("ready", () => { console.log("Connected"); });
...
// Character animation (whole message)
await sdk.messages.sendMessage({
  chatGuid: "iMessage;-;+1234567890",
  message: "Ripple wave!",
...
**Text style properties** (per range): `start`, `end`, `bold`, `italic`, `underline`, `strikethrough`

**Text animations**: `big`, `small`, `shake`, `nod`, `explode`, `ripple`, `bloom`, `jitter`
...
sdk.on("message-send-error", (data) => { console.error("Send failed:", data); });
```
