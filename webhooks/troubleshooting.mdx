...
if (eventId && await store.exists(eventId)) return c.text('ok', 200);
await processOnce(payload);
if (eventId) await store.set(eventId, true, { ttl: 48 * 60 * 60 });
```

If multiple services run their own dedupe table on the same project, scope by webhook id too:
...
`X-Spectrum-Event-Id` is omitted on dynamic-event deliveries (typing indicators, custom provider events) because their payloads don't share a canonical id field across providers — for those, parse the body and pick an event-scoped identifier yourself.
...
console.log(
  `curl -X POST http://localhost:3000/spectrum-webhook \\
    ...
    -H "X-Spectrum-Event-Id: spc-msg-00000000-0000-4000-8000-000000000001" \\
