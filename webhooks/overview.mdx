...
- **Bytes for binary content.** Attachments, voice memos, and contact photos ship `mimeType` / `size` / filename — never the raw bytes themselves and never a download URL. To process the actual content, hold a [`spectrum-ts`](/spectrum-ts/getting-started) instance and call its byte accessors. A first-class HTTP download endpoint is on the roadmap.
...
- **Recursive message trees.** Reactions, replies, and edits ship a slim *reference* to the parent message (`id`, `platform`, `timestamp`, an 80-char `contentPreview`) — not the full parent content. Look the parent up in your own store using `target.id`. See [Events → Target refs](/webhooks/events#target-refs) for the exact shape.
...
Failed deliveries are retried with exponential backoff and jitter — up to 6 attempts spanning tens of seconds of backoff (longer if your endpoint hangs) — then dropped. There is no persistent retry queue or dead-letter destination — see [Delivery and retries](/webhooks/delivery) for the precise policy.
