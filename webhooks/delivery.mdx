...
```ts
const eventId = c.req.header('X-Spectrum-Event-Id');
...
The header carries the same value as `payload.message.id` in the body. Reading it from the header lets dedupe middleware run before JSON parsing — useful when you want to short-circuit duplicates on a hot path or before signature verification on a path that prefers cheap rejection first.

If you need per-URL scoping (multiple services each running their own dedupe table on the same project), compose with `X-Spectrum-Webhook-Id`:

```ts
const dedupeKey = `${webhookId}:${eventId}`;
```

<Note>
`X-Spectrum-Event-Id` is set on `messages` deliveries only. Dynamic-event deliveries (typing indicators, custom provider events) omit the header because their payloads don't share a canonical id field across providers. For those, parse the body and pick an appropriate event-scoped identifier yourself.
</Note>
