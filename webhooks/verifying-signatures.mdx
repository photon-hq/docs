...
2. **Reject the timestamp** if it's more than ±5 minutes (300 seconds) from your current clock.
...
5 minutes (300 seconds) is the recommended tolerance — generous enough to absorb clock drift between our worker and your server, tight enough to make captured-and-replayed attacks impractical. Tighten or loosen it based on your threat model.
...
Spectrum's own delivery worker uses an internal `verifyPhotonWebhook` function that mirrors the algorithm above and bakes in the staleness check. We round-trip our signer against this verifier in tests on every commit, so any bug in the algorithm would be caught before release. You can copy the same shape into your code:
...
export type VerifyResult =
  ...
  | {
      ok: false;
      reason: 'invalid_signature' | 'malformed_timestamp' | 'stale_timestamp';
    };
...
export const verifyPhotonWebhook = (
  ...
  toleranceSeconds: number = DEFAULT_REPLAY_TOLERANCE_SECONDS,
): VerifyResult => {
...
A few details worth knowing about this shape:
...
```ts
const result = verifyPhotonWebhook(rawBody, secret, signature, timestamp);
if (!result.ok) {
  log.warn('webhook verification failed', { reason: result.reason });
  return new Response(result.reason === 'stale_timestamp' ? 'stale' : 'bad signature', {
    status: result.reason === 'stale_timestamp' ? 400 : 401,
  });
}
```
