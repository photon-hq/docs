...
function assemble(
  ...
  for (const block of blocks) {
    if (block.kind === "skip" || block.kind === "interface") continue;
    idx++;
    ...
    if (block.kind === "standalone") {
      ...
      content = [header, config.importPreamble, "", config.declarePreamble, "",
        `async function __check() {`, strippedCode, `}`].join("\n") + "\n";
    } else {
      content = [header, config.importPreamble, "", config.declarePreamble, "",
        `async function __check() {`, code, `}`].join("\n") + "\n";
    }
...
function parseErrors(tscOutput: string, blocks: CodeBlock[], ext: string): TypeErrorInfo[] {
  ...
  let match;
  while ((match = lineRegex.exec(tscOutput)) !== null) {
    const file = basename(match[1]);
    const tscLine = parseInt(match[2], 10);
