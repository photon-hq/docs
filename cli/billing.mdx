...
Creates a Stripe Checkout session and opens it in your browser. With no arguments, the CLI lists available plans and lets you pick one interactively. Pass a `[tier]` positional or `--plan <price-id>` to skip the picker.
