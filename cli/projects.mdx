...
Subscribe a project or manage its existing subscription. The command smart-routes based on current state:

- **Free or unsubscribed projects** → Stripe Checkout
- **Active or past-due subscriptions** → Stripe Customer Portal (change plan, payment method, cancel, etc.)
