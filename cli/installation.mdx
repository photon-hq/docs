...
If you use `npx` / `pnpx` / `bunx`, you automatically pick up new releases — no manual update needed. These tools cache resolved versions, so to force the latest immediately, pin to `@latest`:
