...
Check for accessibility issues in content
Rename/move files and update references
Validate documentation builds

Required files

Site configuration (navigation, theme, integrations, etc.). See [global settings](https://mintlify.com/docs/settings/global) for all options.
files - Documentation pages with YAML frontmatter
...
: Lucide or Font Awesome icon name, URL, or file path.
: Label next to the page title in the sidebar (for example, "NEW").
: Page layout mode (`default`, `wide`, `custom`).
: Array of terms related to the page content for local search and SEO.
...
Auto-generate pages from OpenAPI spec. Add at group/tab level to inherit
...
Frontmatter required** - Every MDX file needs `title` at minimum
Code block language** - Always specify language identifier
Never use `mint.json`** - `mint.json` is deprecated. Only ever use `docs.json`
