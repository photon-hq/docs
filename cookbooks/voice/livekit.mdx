...
- A Photon project with an eligible **SMS number** or **dedicated iMessage
  line**. Shared iMessage assignments do not support Voice.
...
number you want to connect and open its SIP Voice profile.
...
profile affect new calls on every line using it. If your number uses an
additional profile, configure that profile.
...
| --- | --- |
| **Trunk name** | `Photon Voice Demo`, or your own label. |
...
object. Keep the number and media setting you configured. A complete example
looks like this; replace both placeholders before saving:

```json
{
  "name": "Photon Voice Demo",
  "numbers": ["YOUR_PHOTON_NUMBER"],
  "authUsername": "photon-livekit",
...
| LiveKit field | What to enter |
| --- | --- |
| **Rule name** | `Photon inbound calls`, or your own label. |
...
Each incoming call gets a separate room whose name starts with `photon-`.
...
2. Open **Telephony → Calls** in LiveKit and inspect the call's session to find
   its room name. It should start with `photon-` for the rule above.
...
| LiveKit field | What to enter |
| --- | --- |
| **Trunk name** | `Photon Voice outbound`, or your own label. |
